...
export interface ProjectCardProps extends React.HTMLAttributes<HTMLDivElement> {
  ...
  link: string;
  linkText?: string;
  tags?: string[]; // Added to showcase technical precision
}
...
const ProjectCard = React.forwardRef<HTMLDivElement, ProjectCardProps>(
  ({ className, imgSrc, title, description, link, linkText = "View Project", tags, ...props }, ref) => {
    return (
      ...
      >
        {/* Card Image Section */}
        <div className="aspect-video overflow-hidden border-b border-border-subtle">
          <img
            src={imgSrc}
            alt={title}
        ...
        <div className="flex flex-1 flex-col p-6">
          {tags && (
            <div className="flex flex-wrap gap-2 mb-4">
              {tags.map((tag) => (
                <span key={tag} className="text-[10px] font-mono font-bold uppercase tracking-widest text-brand-primary">
                  {tag}
                </span>
              ))}
            </div>
          ...
          <a
            ...
            className="group/button mt-6 inline-flex items-center gap-2 text-sm font-bold text-brand-primary transition-all duration-300 hover:underline"
